...
#include <nori/bsdf/diffuseBSDF.h>
...
Color3f DiffuseBSDF::eval(const BSDFQueryRecord &bRec) const
{
...
float DiffuseBSDF::pdf(const BSDFQueryRecord &bRec) const
{
    /* This is a smooth BRDF -- return zero if the measure
       is wrong, or when queried for illumination on the backside */
    if (bRec.measure != ESolidAngle
...
Color3f DiffuseBSDF::sample(BSDFQueryRecord &bRec, const Point2f &sample) const
{
