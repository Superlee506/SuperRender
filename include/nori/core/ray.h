 ...
 */
template <typename PointType, typename _VectorType> struct TRay {
...
/**
 * \brief Apply normal perturbation when generate ray to avoid self intersection with
 * current triangle. Using shading normal is not correct, you should use geometry normal
 * */
    void applyPositionBias(const VectorType& geometryWorldNormal, const float& maxNormalBias)
    {
        const float minBias = 0.001;
        const float maxBias = std::max(minBias, maxNormalBias);
        const float NormalBias = lerp(maxBias, minBias,
                                      std::clamp(geometryWorldNormal.dot(d), 0.0f, 1.0f));
        o += geometryWorldNormal * NormalBias;
    }
